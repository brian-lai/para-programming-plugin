...
🎯 Next Action:
   Run /para-program:init to set up PARA structure (if not done)
   Or run /para-program:plan to start a new task
```
...
```
/para-program:check "Add JWT-based user authentication to the API"
```
...
```
/para-program:plan add-jwt-authentication
```
...
🎯 Next Action:
   Run /para-program:archive to clean up and prepare for next task
```
...
```
/para-program:plan add-rate-limiting
```
...
1. **Every code change starts with a plan** - `/para-program:plan`
2. **Always get human review** - Before execution
3. **Document everything** - `/para-program:summarize` captures learnings
4. **Clean up when done** - `/para-program:archive` maintains clean context
5. **Stay oriented** - `/para-program:status` shows where you are
